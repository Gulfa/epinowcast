...
model {
  ...
  if (likelihood) {
    ...
    if (ll_aggregation) {
      target += reduce_sum(
        delay_group_lupmf, groups, 1, flat_obs, sl, csl,  exp_lobs, t, sg,
        ts, st, rep_findex, srdlh, ref_lh, refp_findex, model_refp, rep_fncol, ref_as_p, phi, model_obs, model_miss, miss_obs, missing_reference,
