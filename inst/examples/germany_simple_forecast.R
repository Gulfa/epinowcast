...
nowcast <- epinowcast(pobs,
                      expectation = enw_expectation(~ rw(two_weeks_from_last) +(1|day_of_week) , data = pobs),
                      forecast = enw_forecast(~ rw(two_weeks_from_last) +(1|day_of_week)  , N_days=14, data=pobs),
                      fit = enw_fit_opts(
...
summary(nowcast, type="forecast")
enw_forecast_plot(nowcast)
